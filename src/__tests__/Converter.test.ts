...
describe('NavDrawer component', () => {
  ...
  test('converter generates correct files', async () => {
    ...
    const editorButton = await driver.findElement(By.id("Editor"));
    await editorButton.click();

    //find the "open" button (second from the left on the upper bar) and then click it
    const openButton = await driver.findElement(By.id("openButton"));
    await openButton.click();
    
    //wait for the "open project" window to appear
    const openProjectWidget = await driver.wait(until.elementLocated(By.id("openProjectDialog")), 10000);
    ...
    const loadButton = await openProjectWidget.findElement(By.id("loadExampleButton"));
    ...
    const filesButton = await driver.findElement(By.id("Input files"));
    ...
    await driver.wait(until.elementTextMatches(driver.findElement(By.id('geo.datTextField')), /\S/));
    ...
    const geoText = (await driver.findElement(By.id('geo.datTextField')).getText()).replace(regex, '');
    ...
    const matText = (await driver.findElement(By.id('mat.datTextField')).getText()).replace(regex, '');
    ...
    const beamText = (await driver.findElement(By.id('beam.datTextField')).getText()).replace(regex, '');
    ...
    const detectText = (await driver.findElement(By.id('detect.datTextField')).getText()).replace(regex, '');
