...
export const incrementName = (name: string, until: (name: string) => boolean) => {
    if (until(name)) {
        ...
        if (isNaN(nameNumber)) {
            nameNumber = 1
            if (lastInName.length > 0)
                nameBase = `${nameBase}_${lastInName}`
        }
...
export const implementsUniqueChildrenNames = (x: unknown): x is UniqueChildrenNames => { return typeof x === 'object' && x !== null && 'getNextFreeName' in x; }
