...
function SidebarMaterial(editor) {
	...
	const materialAttenuationTint = new SidebarMaterialColorProperty(
		editor,
		'attenuationTint',
		strings.getKey('sidebar/material/attenuationTint')
	);
	...
	function update() {
		...
		if (material) {
			if (material.uuid && material.uuid !== materialUUID.getValue()) {
				editor.execute(
					new SetMaterialValueCommand(
						editor,
						currentObject,
