...
UI can be deployed on different machine (with different IP) than a backend. During build phase the UI can be configured to talk to given backend instance via `VITE_REACT_APP_BACKEND_URL` environmental variable. To adjust it put following line in the `.env` file:
...
- `VITE_REACT_APP_TARGET` - if set to `demo`, app will not require authentication and will be preloaded with demo results (this version is available at <https://yaptide.github.io/web_dev/>)
- `VITE_REACT_APP_ALT_AUTH` - if set to `plg`, app will use plgrid authentication
- `VITE_REACT_APP_DEPLOYMENT` - if set to `dev`, configuration wil be editable from the browser console. For example, you can change the backend URL by typing `window.BACKEND_URL="http://mynew.url"` in the browser console.
