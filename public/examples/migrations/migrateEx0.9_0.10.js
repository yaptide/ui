...
const checkEmpty = obj =>
	Object.entries(obj).forEach(([key, item]) => {
		if (item === undefined) throw new Error(`Missing ${key} in ${JSON.stringify(obj)}`);
	});
...
const migrateInputJson = inputJson => {
	...
	const figureManager = {
		...
		figures: oldScene.object.children.map(child => {
			const { name, type: meshType, uuid, geometryData: partialGeometryData } = child;
			...
			return {
				...
				geometryData: {
					...partialGeometryData,
					geometryType:
						partialGeometryData.geometryType === 'CylinderGeometry'
							? 'HollowCylinderGeometry'
							: partialGeometryData.geometryType
				},
				colorHex: 0
			};
		})
		...
		zones: oldZoneManager.zones.map(zone => {
			return {
				...
				customMaterial: zone.customMaterial
					? {
							...zone.customMaterial,
							originalMaterialUuid: materialManager.materials.find(
								material => material.icru === zone.customMaterial.icru
							).uuid
						}
					: undefined
			};
	...
	const detectorManager = {
		...
		detectors: oldDetectManager.detectGeometries.map(detector => {
			const {
				uuid,
				name,
				type: geometryType,
...
if (!fs.existsSync('./migrations/temp')) fs.mkdirSync('./migrations/temp');

fs.writeFileSync(
	`./migrations/temp/${Date.now()}_${fileNameToMigrate}`,
